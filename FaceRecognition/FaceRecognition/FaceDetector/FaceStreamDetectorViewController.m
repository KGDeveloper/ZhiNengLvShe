...
//    [self.view addSubview:imgView];
    ...
    self.textLabel = [[UILabel alloc]initWithFrame:CGRectMake((ScreenWidth-150)/2, CGRectGetMaxY(self.previewView.frame)+50, 150, 30)];
...
#pragma mark ------------------------------在这里添加了长传照片---------------------------------------------
            [self sendImageToServer:myImage];
#pragma mark ------------------------------在这里添加了长传照片---------------------------------------------
            //停止摄像
            [self.previewLayer.session stopRunning];
...
#pragma mark -开始上传照片-
- (void)sendImageToServer:(UIImage *)dataImage{
    ...
    AVCaptureViewController *vontroller = [[AVCaptureViewController alloc]init];
    
    [self.navigationController pushViewController:vontroller animated:YES];
...
//        NSData *eachImgData = UIImageJPEGRepresentation(dataImage, 0.5);
//        [formData appendPartWithFileData:eachImgData name:@"original_img" fileName:[NSString stringWithFormat:@"imageg.jpg"] mimeType:@"image/jpg"];
